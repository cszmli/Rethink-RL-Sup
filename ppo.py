...
from utils import state_vectorize, to_device, domain_vectorize
...
class PPO(object):
    ...
    def domain_loss(self, data):
        ...
        if self.train_direc=='opposite':
            loss_d = - loss_d
        return loss_d.mean().item() * self.mt_factor
    ...
    def update(self, batchsz, epoch, best=None):
        ...
        for i in range(self.update_round):
            ...
            v_target_shuf, A_sa_shuf, s_shuf, a_shuf, log_pi_old_sa_shuf, d_shuf = v_target[perm], A_sa[perm], s[perm], a[perm], \
                                                                           log_pi_old_sa[perm], d[perm]

            # 2. get mini-batch for optimizing
            optim_chunk_num = int(np.ceil(batchsz / self.optim_batchsz))
            # chunk the optim_batch for total batch
            v_target_shuf, A_sa_shuf, s_shuf, a_shuf, log_pi_old_sa_shuf, d_shuf = torch.chunk(v_target_shuf, optim_chunk_num), \
            ...
            for v_target_b, A_sa_b, s_b, a_b, log_pi_old_sa_b, d_b in zip(v_target_shuf, A_sa_shuf, s_shuf, a_shuf,
                                                                     log_pi_old_sa_shuf, d_shuf):
                ...
                domain_loss = self.domain_loss([s_b, a_b, d_b])
                surrogate += domain_loss

                domain_loss_hist += domain_loss
